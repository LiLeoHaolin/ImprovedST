...
```{r}
lung = read_csv("/Users/Haolin Li/Desktop/tree/03_applications/S1Data.csv")
tau = max(lung$time)
tau
sum(subset(lung,time<250)$status==1)/nrow(subset(lung,time<250))
nrow(subset(lung,status==0))
```

```{r}
structure=TRUE
model1=ranger(Surv(time, status) ~ .,data=lung,num.tree=500,mtry=ncol(lung)-2,splitrule = "logrank", min.node.size = 50)

set.seed(109) # seed 109, num_leaf=4, min leaf size=13
# seed 7305, min leaf size=11

model2=ranger(Surv(time, status) ~ .,data=lung,num.tree=1,mtry=ncol(lung)-2,splitrule = "logrank", min.node.size = 50) # sample without rep
...
plot(seq(tau/num_interval,tau,tau/num_interval),result,type="l",xlab="Time", ylab="Survival Probability",ylim=c(0,1),main="Terminal Node 2")
```
