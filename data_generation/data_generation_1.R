...
path.cc <- "/nas/longleaf/home/fyy2025/survival_tree/tree_Ld_Me_Ls/data/"
...
# # high-dimensional tree-based DGP
# p=10
# for (j in 1:nsim){
#   cat(j)
#   X = as.matrix(mvrnorm(n = n, matrix(0, nrow = p, ncol = 1), ar1_cor(p, 0.6))) #mvrnorm: multivariate normal dist
#   #mu: a vector of 0 with length p-- means of variables
#   #Sigma: a positive-definite symmetric matrix specifying the covariance matrix of the variables.
#   true.lin.pred <- 0.3*(X[,1]>0) -0.2*(X[,2]<0)
#   v <- runif(n=n)
#   Tlat <- (- log(v) / (1* exp(true.lin.pred)))^(1 / gamma.weibull)
#   # C = Tlat
#   # censor_index = sample(n, n/2, replace = FALSE)
...
# 
# 
# # low-dimensional linear covariate DGP
# # p=4
# for (j in 1:nsim){
#   cat(j)
#   X = as.matrix(mvrnorm(n = n, matrix(0, nrow = p, ncol = 1), ar1_cor(p, 0.6))) #mvrnorm: multivariate normal dist
#   #mu: a vector of 0 with length p-- means of variables
#   #Sigma: a positive-definite symmetric matrix specifying the covariance matrix of the variables.
#   true.lin.pred <- 0.3*X[,1] -0.2*X[,2]+ 0.1*X[,3] -0.9*X[,4]-0.3
#   v <- runif(n=n)
#   Tlat <- (- log(v) / (1* exp(true.lin.pred)))^(1 / gamma.weibull)
#   # C = Tlat
#   # censor_index = sample(n, n/2, replace = FALSE)
...
# 
# 
# # high-dimensional linear covariate DGP
# p=10
# for (j in 1:nsim){
#   cat(j)
#   X = as.matrix(mvrnorm(n = n, matrix(0, nrow = p, ncol = 1), ar1_cor(p, 0.6))) #mvrnorm: multivariate normal dist
...
# 
# 
# # low-dimensional nonlinear covariate DGP
# p=4
# for (j in 1:nsim){
#   cat(j)
#   X = as.matrix(mvrnorm(n = n, matrix(0, nrow = p, ncol = 1), ar1_cor(p, 0.6))) #mvrnorm: multivariate normal dist
#   #mu: a vector of 0 with length p-- means of variables
#   #Sigma: a positive-definite symmetric matrix specifying the covariance matrix of the variables.
#   true.lin.pred <- 0.3*X[,1] -0.2*X[,2]+ 0.1*X[,3] -0.9*X[,4]-0.3 - 1.6*X[,4]^2-0.3 + X[,2]*X[,3]
#   v <- runif(n=n)
#   Tlat <- (- log(v) / (1* exp(true.lin.pred)))^(1 / gamma.weibull)
#   # C = Tlat
#   # censor_index = sample(n, n/2, replace = FALSE)
...
# 
# 
# # high-dimensional nonlinear covariate DGP
# p=10
# for (j in 1:nsim){
#   cat(j)
#   X = as.matrix(mvrnorm(n = n, matrix(0, nrow = p, ncol = 1), ar1_cor(p, 0.6))) #mvrnorm: multivariate normal dist
