...
tau = 0.0005
path.cc <- "/nas/longleaf/home/fyy2025/survival_tree/nl_300_20/"

num_dataset=500
initial_loss=rep(0,num_dataset)
super_loss=rep(0,num_dataset)
forest_loss=rep(0,num_dataset)
leaf_num=rep(0,num_dataset)
event_rate=rep(0,num_dataset)
for (dataset in 1:num_dataset){
  train = read_csv(paste0(path.cc,"data/train",dataset,".csv"))
  test = read_csv(paste0(path.cc,"data/test",dataset,".csv"))
  structure=TRUE
  ...
  leaf[[1]] <- chf_value[1,] #this is the chf value for the first observation
  ...
  # treeInfo(model2,1)
  # Reassigning the nodes with too few observations
  leaf_size=as.vector(table(group))
  single_leaf=as.vector(which(leaf_size<=4))
  normal_leaf=as.vector(which(leaf_size>4))
  ...
  leaf_num[dataset]=num_leaf
  event_rate[dataset] = table(train$status)[2]/nrow(train)
}

write.csv(as.matrix(data.frame(forest_loss, initial_loss,super_loss,leaf_num,event_rate)),file=paste0(path.cc, 'results/summary.csv'), row.names = F)
...
# write.csv(as.matrix(leaf_num),file=paste0(path.cc, 'results/super_loss.csv'), row.names = F)
